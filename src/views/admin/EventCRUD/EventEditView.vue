...
onMounted(async () => {
  ...
  form.show = String(eventStore.event.show);
  form.theater = String(eventStore.event.theater);
  form.event_date = eventStore.event.event_date
    ? eventStore.event.event_date.slice(0, 16) // datetime-local format
    : "";
  form.sales_start = eventStore.event.sales_start
    ? eventStore.event.sales_start.slice(0, 16)
    : "";

  form.sales_end = eventStore.event.sales_end
    ? eventStore.event.sales_end.slice(0, 16)
    : "";
});
...
<template>
  <div class="mx-auto max-w-3xl">
    <div class="rounded-2xl border border-[#2B223A] bg-[#1A1427] p-8">
      <h1 class="mb-8 text-3xl font-bold text-white">Edit Event</h1>
